...
The ack is per zone, not per connection: each subscribed zone keeps its own record of your highest consumed `seq` and acks from it on its own broadcast schedule, and nothing in the frame says which zone sent it. Once you have crossed a boundary and stamped an input on the far side, every subscribed zone holding a record for you is acking, so more than one `world.ack` reaches you per tick. Only the zone you are in advances its record; the one you left keeps repeating the mark it froze when you were there, so `payload.seq` can go backwards from one ack to the next. Your own counter never goes backwards; what you receive does. Known wart, tracked as [widgrensit/asobi#477](https://github.com/widgrensit/asobi/issues/477).
...
- Acks come from each subscribed zone separately, so after a crossing more than one reaches you per tick and their `seq` values can arrive out of order. Keep the running maximum the sample keeps ([widgrensit/asobi#477](https://github.com/widgrensit/asobi/issues/477)).
